...
Commit generated updates under `.claude-plugin/marketplace.json`, `.agents/plugins/marketplace.json`, `catalog.md`, and `site/` with your PR.

> **Codex:** A plugin's skills appear under Codex only if its source repo ships a manifest (`.codex-plugin/plugin.json`, or a legacy `.claude-plugin/plugin.json`) with a `skills` path. A `strict: false` plugin with no manifest installs with zero skills under Codex — add a `.codex-plugin/plugin.json` (with a `skills` path) upstream to fix it. Codex reads the registry's `.agents/plugins/marketplace.json`, generated by `scripts/sync_codex_marketplace.py`.
