...
> **Codex:** A plugin's skills appear under Codex only if its source repo ships a `.codex-plugin/plugin.json` manifest with a `skills` path. A `strict: false` plugin with no such manifest installs with zero skills under Codex — add a `.codex-plugin/plugin.json` (with a `skills` path) upstream to fix it. Codex reads the registry's `.agents/plugins/marketplace.json`, generated by `scripts/sync_codex_marketplace.py`.
